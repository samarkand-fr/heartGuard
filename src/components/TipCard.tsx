...
const TipCard: React.FC<TipCardProps> = ({ tip }) => {
  // Normalisation de l'icône pour s'assurer qu'elle corresponde exactement à la clé de iconMap
  const normalizedIcon = tip.icon.charAt(0).toUpperCase() + tip.icon.slice(1); // Première lettre en majuscule

  return (
    <div className="flex items-center space-x-4 bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-gray-200">
      <div>{iconMap[normalizedIcon] || <HeartPulse size={32} className="text-gray-400" />}</div>
      <p className="text-gray-800 text-lg">{tip.tip}</p>
    </div>
  );
};
